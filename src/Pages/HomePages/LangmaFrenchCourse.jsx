...
const TESTIMONIALS = [
  {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789032944/Advice_Mobile_Video_in_Experimental_Type_Style_qmxxzi.mp4"
  },
  {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789033823/French_bn7plu.mp4"
  },
  {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789033825/Advice_Mobile_Video_in_Experimental_Type_Style_1_v1ecf0.mp4"
  },
  {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789033856/Advice_Mobile_Video_in_Experimental_Type_Style_2_oxzvav.mp4"
  },
  {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789034571/French_1_rwoxn3.mp4"
  },
    {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789035433/French_2_gacqvt.mp4"
  },
      {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789035829/French_3_oxei8x.mp4"
  },
        {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789373649/Feedback_French_Poonam_bgh9tz.mov"
  },
        {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789373659/fRENCH_QUESTION_WORD_xdbvkb.mp4"
  },
        {
    video: "https://res.cloudinary.com/dzv9zcrlz/video/upload/v1789373703/French_Tongue_twister_ejicwi.mp4"
  },
];
...
// French language certification exam badges shown in the hero exam strip
const EXAM_BADGES = [
  { code: "DELF", subtitle: "A1–B2, Worldwide", ring: "#0055A4", accent: "#EF4135", letter: "D" },
  { code: "DALF", subtitle: "C1–C2, Advanced", ring: "#1A1A1A", accent: "#0055A4", letter: "D" },
  { code: "TCF", subtitle: "French Proficiency", ring: "#EF4135", accent: "#0055A4", letter: "T" },
  { code: "TEF", subtitle: "Study & Immigration", ring: "#0055A4", accent: "#1A1A1A", letter: "T" },
  { code: "DFP", subtitle: "Business French", ring: "#D4AF37", accent: "#0055A4", letter: "P" },
  { code: "Campus France", subtitle: "Study Pathway", ring: "#EF4135", accent: "#1A1A1A", letter: "C" }
];
...
const FRENCH_FORM_CONFIG = {
  language: "French",
  destinationCode: "FRA",
  destinationLabel: "Your Future, France",
  message: "French Language Course enquiry",
  type: "French Landing",
  service: "Language Training - French",
};
...
export default function LangmaFrenchCourse() {
  ...
  async function submitLead(form, phoneValue, { setError, setSubmitted, setBusy, setMessage }) {
    ...
    formData.set("language", FRENCH_FORM_CONFIG.language);
    formData.set("type", FRENCH_FORM_CONFIG.type);
    formData.set("service", FRENCH_FORM_CONFIG.service);
    if (!formData.get("message")) {
      formData.set("message", FRENCH_FORM_CONFIG.message);
    }
      ...
      try {
        const fallback = await fetch(`${API_BASE}/api/contact-lead`, {
          ...
          body: JSON.stringify({
            ...
            mobile: phoneValue,
            message: formData.get("message") || FRENCH_FORM_CONFIG.message,
            type: FRENCH_FORM_CONFIG.type,
            service: FRENCH_FORM_CONFIG.service,
          }),
        });
        if (fallback.ok) {
          setSubmitted(true);
  ...
  async function handleRequestSubmit(e) {
    ...
    setRequestFormMessage("");
    const typeMessages = {
      demo: "Interested in Learning French - Request Demo",
      study: "Interested in Study in France Program",
      work: "Interested in Work in France Pathway",
      ssw: "Interested in Talent Passport (Skilled Worker) Program",
    ...
    try {
      const res = await fetch(`${API_BASE}/api/contact-lead`, {
        ...
        body: JSON.stringify({
          ...
          message: typeMessages[requestPopupType] || "French Course Inquiry",
          currenturl: window.location.href,
          language: "French",
          type: "French Landing - Request Form"
        }),
      });
      if (res.ok) {
  ...
  * { min-width: 0; }
  ...
  .tricolor-grid-dark{background-image:linear-gradient(90deg,rgba(0,85,164,.16) 0 33.33%,rgba(255,255,255,.08) 33.33% 66.66%,rgba(239,65,53,.16) 66.66%); background-size:84px 84px;}
  ...
  .fr-header{background:rgba(252,253,255,.96); backdrop-filter:blur(10px); border-bottom:1px solid var(--line); position:fixed; top:var(--announce-h, 0px); left:0; right:0; width:100%; height:var(--header-h); z-index:80; box-shadow:0 4px 18px rgba(19,39,56,.08); transition:top .25s ease;}
  ...
  .hero-photo{position:absolute; inset:0; background-image:linear-gradient(100deg, rgba(252,253,255,.985) 0%, rgba(252,253,255,.96) 48%, rgba(252,253,255,.78) 68%, rgba(252,253,255,.38) 100%), url('https://images.unsplash.com/photo-1502602898657-3e91760cbb34?fm=jpg&q=80&w=1800&auto=format&fit=crop'); background-size:cover; background-position:center 65%; opacity:.03;}
  ...
  /* Ticket visual — the hero's signature element: a boarding pass to France */
  ...
  .tricolor-stamp{position:absolute; top:18px; right:18px; width:56px; height:56px; border:2px solid var(--rust); border-radius:8px; color:#fff; background:var(--rust); display:flex; align-items:center; justify-content:center; transform:rotate(-8deg); z-index:3; box-shadow:0 6px 16px rgba(190,30,45,.4); pointer-events:none;}
  ...
  .activity-image{width:100%; aspect-ratio:5/0; height:auto; object-fit:cover; object-position:center; display:block; flex-shrink:0; background:#142536; transition:transform .5s var(--ease); will-change:transform; contain:layout style paint;}
  ...
  `}</style>
      ...
      {showAnnouncement && (
        <div className="announcement-bar" ref={announceRef}>
          <div className="announcement-inner">
            <span className="announcement-text">🇫🇷 New French batch enrolling now — few seats available!</span>
            <a className="announcement-link" href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20to%20reserve%20a%20seat%20in%20the%20new%20French%20batch." target="_blank" rel="noopener">Reserve My Seat →</a>
          </div>
      ...
      <header className="fr-header">
        <div className="wrap fr-nav">
          <a className="fr-brand" href="#top" aria-label="Langma French Course">
          ...
          <nav className="fr-menu" aria-label="French course navigation">
      ...
      <section className="hero">
        ...
        <div className="hero-inner">
          <div>
            <span className="hero-kicker">French Language Course · Online &amp; Offline Classes</span>
            <h1 className="hero-title">Your Gateway to France. Begins with <span className="heading-accent">French Language</span>.</h1>
            <p className="hero-sub">Langma helps students and professionals master French with expert trainers, small batch learning, DELF preparation, and practical support for study in France, work in France, and visa pathways. Choose flexible online or offline French classes in {locationProfile.city} and build real fluency for your future in France.</p>
            <div className="hero-actions">
              <a href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20a%20free%20French%20demo%20class." target="_blank" rel="noopener" className="btn btn-wa">
              ...
              <a href="tel:+919810117094" className="btn btn-ghost">Talk to a French Counsellor</a>
            </div>
            <div className="hero-stats">
              ...
              <div className="stat">
                ...
                <div className="stat-text"><b>A1–C2</b><span>ALL DELF LEVELS</span></div>
              </div>
              <div className="stat">
            ...
            <div className="ticket-top">
              <div className="ticket-route">
                ...
                <div style={{textAlign: 'right'}}>
                  <div className="city">{FRENCH_FORM_CONFIG.destinationCode}</div>
                  <div className="sub">{FRENCH_FORM_CONFIG.destinationLabel}</div>
                </div>
              </div>
              ...
              <form className="ticket-form" onSubmit={handleHeroSubmit}>
                ...
                <input type="hidden" name="language" value={FRENCH_FORM_CONFIG.language} />
                <input type="hidden" name="message" value={FRENCH_FORM_CONFIG.message} />
                <input type="hidden" name="type" value={FRENCH_FORM_CONFIG.type} />
                <input type="hidden" name="service" value={FRENCH_FORM_CONFIG.service} />
        ...
        {/* ===== FRENCH EXAM CERTIFICATION LOGOS STRIP ===== */}
        <div className="exam-strip">
          <div className="wrap">
            <div className="exam-head">
              <span className="kicker">DELF, DALF, TCF &amp; Career Readiness</span>
              <h3>Prepare for the exam and the life you want in France.</h3>
            </div>
            <div className="exam-row reveal-group" role="list" aria-label="French language certification exams we prepare you for">
              {EXAM_BADGES.map((b, i) => {
                return (
        ...
        <div className="wrap">
          ...
          <div className="audience-grid reveal-group">
            ...
            <div className="audience-card">
              ...
              <p>Build strong French skills for university admissions, language programs, and long-term academic goals.</p>
            ...
            <div className="audience-card">
              ...
              <p>Prepare for workplace communication, interview confidence, and career pathways in France.</p>
            ...
            <div className="audience-card">
              ...
              <div className="audience-num">Talent Passport</div>
              <h3>Skilled Employment</h3>
              <p>Build the French language foundation needed for sector-specific work, test preparation, and skilled migration pathways.</p>
              <button className="btn btn-primary btn-sm" onClick={() => openRequestPopup('ssw')} style={{marginTop: '14px', width: '100%', justifyContent: 'center', cursor: 'pointer'}}>Request Talent Passport Program Info</button>
            </div>
            <div className="audience-card">
              ...
              <p>Learn practical French for travel, conversation, French media and cultural understanding.</p>
          ...
          <div className="section-cta-big">
              <h3 className="section-cta-heading">Turn your French into your next opportunity</h3>
            <p className="section-cta-subtext">Choose a learning path for study, work, travel or confident everyday communication.</p>
            <a className="btn btn-primary btn-large" href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20help%20choosing%20the%20right%20French%20learning%20path." target="_blank" rel="noopener">
      ...
      <section id="skills">
        <div className="wrap">
          <div className="sec-head reveal">
            <h2>Build real <span className="heading-accent">French</span> skills for everyday life and work.</h2>
            <p>Our French curriculum develops speaking, listening, reading, and writing together so you can communicate confidently in real situations, not just in a classroom.</p>
          </div>
          <div className="skills-grid reveal-group">
            <div className="skill-stamp">
              <div className="skill-native">P</div>
              <span className="skill-en">Parler · Speaking</span>
              <p>Guided conversation practice from day one, so you're comfortable speaking long before you've memorised every grammar rule.</p>
            </div>
            <div className="skill-stamp">
              <div className="skill-native">E</div>
              <span className="skill-en">Écouter · Listening</span>
              <p>Native-accent audio and dialogues that train your ear for natural spoken French, not textbook French.</p>
            </div>
            <div className="skill-stamp">
              <div className="skill-native">L</div>
              <span className="skill-en">Lire · Reading</span>
              <p>From the French alphabet to everyday vocabulary, building comprehension word by word, level by level.</p>
            </div>
            <div className="skill-stamp">
              <div className="skill-native">É</div>
              <span className="skill-en">Écrire · Writing</span>
              <p>Spelling, sentence structure, and the shift from casual to formal register, corrected and improved as you go.</p>
            </div>
          </div>
          <div className="section-cta-big">
            <h3 className="section-cta-heading">Build all four skills together</h3>
            <p className="section-cta-subtext">Speaking, listening, reading &amp; writing—master French comprehensively with expert guidance.</p>
            <a className="btn btn-wa btn-large" href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20to%20book%20a%20free%20French%20demo%20class." target="_blank" rel="noopener">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12.04 2c-5.5 0-9.96 4.46-9.96 9.96 0 1.76.46 3.45 1.32 4.95L2 22l5.25-1.38a9.9 9.9 0 0 0 4.79 1.22h.01c5.5 0 9.96-4.46 9.96-9.96S17.54 2 12.04 2zm5.86 14.13c-.25.7-1.45 1.34-2 1.42-.51.08-1.15.11-1.86-.12-.43-.14-.98-.32-1.69-.63-2.97-1.28-4.9-4.27-5.05-4.47-.15-.2-1.22-1.62-1.22-3.09 0-1.47.77-2.19 1.05-2.49.27-.3.6-.37.8-.37.2 0 .4 0 .58.01.19.01.44-.07.68.53.25.6.85 2.08.92 2.23.07.15.12.32.02.52-.1.2-.15.32-.3.49-.15.17-.31.38-.44.51-.15.15-.3.31-.13.6.17.3.75 1.25 1.62 2.02 1.11.99 2.05 1.3 2.35 1.45.3.15.47.12.65-.07.18-.19.75-.87.95-1.17.2-.3.4-.25.66-.15.27.1 1.73.82 2.02.97.3.15.5.22.57.35.07.13.07.75-.18 1.45z"/></svg>
              Book Your Free Demo
            </a>
      ...
      <section className="journey" id="journey">
        ...
        <div className="wrap">
          <div className="sec-head reveal">
            <h2>Six Steps to <span className="heading-accent">French Language</span> Fluency.</h2>
          ...
          <div className="rail-wrap">
            <div className="rail reveal-group">
              <div className="stop">
                ...
                <h3>Foundations</h3>
                <p>French pronunciation, basic grammar and greetings: everyday phrases you'll actually use from week one.</p>
              ...
              <div className="stop">
                ...
                <span className="unlock-tag">Unlocks: Talent Passport visa eligibility (TCF/A2)</span>
              </div>
              <div className="stop">
              ...
              <div className="stop">
                ...
                <p>French used in a broad range of everyday and workplace scenes: news, meetings, real conversation.</p>
              ...
              <div className="stop">
                ...
                <p>Highly complex, formal and abstract French: read, listened to and understood the way educated native speakers do.</p>
              ...
              <div className="stop">
                ...
                <p>Formal register, boardroom French and the etiquette DELF alone doesn't test: full executive fluency.</p>
      ...
      <section className="study-hours" id="study-hours">
        <div className="wrap">
          <div className="sec-head reveal">
            ...
            <p>Approximate cumulative self-study hours to reach each DELF level from zero. Your actual pace depends on prior exposure, study method and hours committed per week.</p>
          </div>
      ...
      {/* COURSE CURRICULUM */}
      <section id="curriculum"><div className="wrap"><div className="sec-head reveal"><h2>From Learning French to Living Your France Dream.</h2><p>The learning journey develops grammar, vocabulary, comprehension and communication together as you progress from A1 towards advanced French.</p></div><div className="curriculum-grid reveal-group"><div className="curriculum-card"><div className="curriculum-level">Level 01 · A1</div><h3>Foundations</h3><p>Build the base you need to start understanding and using French.</p><ul><li>French alphabet &amp; pronunciation</li><li>Basic grammar &amp; sentence patterns</li><li>Greetings &amp; introductions</li><li>Everyday vocabulary</li><li>Basic listening &amp; speaking practice</li></ul></div><div className="curriculum-card"><div className="curriculum-level">Level 02 · A2</div><h3>Everyday French</h3><p>Move from basic phrases into more independent everyday communication.</p><ul><li>Expanded grammar &amp; vocabulary</li><li>Daily-life conversations</li><li>Reading comprehension practice</li><li>Listening comprehension</li><li>DELF / TCF preparation</li></ul></div><div className="curriculum-card"><div className="curriculum-level">Level 03 · B1</div><h3>Intermediate Bridge</h3><p>Develop longer conversations and wider real-world French.</p><ul><li>Intermediate grammar</li><li>Workplace vocabulary</li><li>Longer reading passages</li><li>Natural conversation practice</li><li>Exam-focused mock practice</li></ul></div><div className="curriculum-card"><div className="curriculum-level">Level 04 · B2</div><h3>Business-Level French</h3><p>Build stronger comprehension and communication for professional environments.</p><ul><li>Advanced grammar &amp; vocabulary</li><li>News &amp; workplace French</li><li>Meetings &amp; professional scenarios</li><li>Formal communication</li><li>DELF B2 preparation</li></ul></div><div className="curriculum-card"><div className="curriculum-level">Level 05 · C1</div><h3>Advanced Mastery</h3><p>Handle complex written and spoken French with greater accuracy.</p><ul><li>Complex grammar structures</li><li>Advanced vocabulary &amp; idiomatic expressions</li><li>Abstract &amp; formal French</li><li>High-level reading &amp; listening</li><li>DELF C1 preparation</li></ul></div><div className="curriculum-card"><div className="curriculum-level">Beyond C1</div><h3>Business &amp; formal register</h3><p>Go beyond exam French into professional and high-context communication.</p><ul><li>Formal register &amp; vouvoiement (polite address)</li><li>Business etiquette</li><li>Professional conversations</li><li>Presentation &amp; meeting language</li><li>Executive communication practice</li></ul></div></div></div></section>
      ...
      {false && <section className="lang-facts" id="language">
        <div className="wrap">
          <div className="lang-inner">
            <div>
              <h2 style={{fontSize: 'clamp(30px,3.4vw,44px)', margin: '0 0 20px', lineHeight: '1.16'}}>French: more useful than most people expect.</h2>
              <div className="lang-copy">
                <p>French is spoken by more than 300 million people across five continents, one of the few major world languages where nearly every speaker lives across Europe, Africa, the Americas and beyond.</p>
                <p>It belongs to the Romance language family, alongside Spanish, Italian and Portuguese, and English borrowed thousands of words from French after 1066. That shared history means far more vocabulary looks familiar from day one than most learners expect, and French word order is close enough to English that the biggest early adjustment is pronunciation and rhythm, not grammar.</p>
                <p>France is a global centre for fashion, gastronomy, aerospace and luxury goods, and a growing hub for finance and technology, which means learning French isn't just a classroom exercise, it's a career move, opening doors in engineering, IT, hospitality and skilled-trade roles across the country.</p>
              </div>
              <div className="cognates">
                <div className="cognates-head"><div>French</div><div>Sounds like</div><div>English</div></div>
                <div className="cognate-row"><div>important</div><div>am-por-tahn</div><div>Important</div></div>
                <div className="cognate-row"><div>restaurant</div><div>res-to-rah</div><div>Restaurant</div></div>
                <div className="cognate-row"><div>famille</div><div>fa-mee</div><div>Family</div></div>
                <div className="cognate-row"><div>camera</div><div>ka-me-ra</div><div>Camera</div></div>
              </div>
            </div>
            <div>
              <img className="lang-photo" src="/images/french-lettering-feature.jpg" alt="French lettering practice" loading="lazy" />
              <div className="fact-strip reveal-group">
                <div className="fact-box"><b>300M+</b><span>speakers worldwide</span></div>
                <div className="fact-box"><b>5</b><span>continents with French-speaking communities</span></div>
                <div className="fact-box"><b>Global</b><span>language of culture, diplomacy and business</span></div>
                <div className="fact-box"><b>Romance</b><span>many familiar words for English speakers</span></div>
              </div>
              <div className="lang-note">
                <h4>Good to know</h4>
                <p>French pronunciation has its own musical rhythm, and confidence grows quickly through regular listening and speaking. <em style={{fontFamily: "'Roboto', sans-serif", fontStyle: 'normal', fontWeight: 700}}>Many words</em> will already feel familiar to English speakers.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="wrap">
          <div className="features-grid reveal-group">
            <div className="feature">
              ...
              <p>Experienced, well-trained instructors fluent in French ensure you get the highest quality instruction, not a script.</p>
            </div>
            <div className="feature">
              ...
              <p>Experience French etiquette, workplace culture and daily life through real-life scenarios that prepare you for actually living in France.</p>
            </div>
            <div className="feature">
              ...
              <p>Exclusive support for the Skilled Employment visa, Engineer/Specialist roles, and in-demand IT and manufacturing jobs in France, right after your course.</p>
            </div>
            <div className="feature">
              ...
              <p>For learners who need a certificate, we prepare you for DELF, TCF and DFP, targeting the exact score your visa or employer requires.</p>
            </div>
            <div className="feature">
      ...
      <section className="modes" id="modes">
        <div className="wrap">
          ...
          <img className="modes-photo" src="https://res.cloudinary.com/dzv9zcrlz/image/upload/v1789023092/french_classes_ozpdef.png" alt="Students in a French language course classroom listening to their instructor" loading="lazy" />
      ...
      <section className="testimonials" id="testimonials">
        <div className="wrap">
          <div className="sec-head reveal">
            ...
            <p>Real stories from learners who have made their move to France. Watch how Langma transformed their French journey.</p>
          </div>
                ...
                {TESTIMONIALS.map((t, i) => (
                  <div className="testi-slide" key={i} style={{ width: `${testiWidthPct}%` }}>
                    <div className="testi-card">
                      <video className="testi-video-el" src={t.video} poster={getVideoPoster(t.video)} controls playsInline preload="none" aria-label="Langma French course student testimonial" />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* WHAT YOU GET */}
      <section className="included" id="included"><div className="wrap"><div className="sec-head reveal"><h2>Everything included in your French course.</h2><p>Support is designed around both language development and the goal behind your French learning journey.</p></div><div className="included-grid reveal-group"><div className="included-item"><span className="tick">✓</span><span>Live instructor-led French classes</span></div><div className="included-item"><span className="tick">✓</span><span>A1–C2 structured learning pathway</span></div><div className="included-item"><span className="tick">✓</span><span>Speaking, listening, reading &amp; writing practice</span></div><div className="included-item"><span className="tick">✓</span><span>Vocabulary and grammar development</span></div><div className="included-item"><span className="tick">✓</span><span>French exam preparation</span></div><div className="included-item"><span className="tick">✓</span><span>Mock tests &amp; progress assessment</span></div><div className="included-item"><span className="tick">✓</span><span>Cultural &amp; real-life communication practice</span></div><div className="included-item"><span className="tick">✓</span><span>Online, offline &amp; hybrid learning options</span></div><div className="included-item"><span className="tick">✓</span><span>Goal-based counselling for France pathways</span></div></div></div></section>

      {sectionFormCta}

      {/* Talent Passport / ENGINEER / STUDY PATHWAYS */}
      <section className="pathways" id="pathways">
        <div className="wrap">
          <div className="sec-head reveal">
            <h2>Explore Study, Work & Career Pathways in France.</h2>
            <p>Langma's counsellors don't stop at fluency. We guide candidates through France work, study, and skilled worker pathways suited to their goals and qualifications.</p>
          </div>
          <div className="path-grid reveal-group">
            <div className="path-card">
              <div className="path-photo-wrap"><img className="path-photo" src="https://res.cloudinary.com/dzv9zcrlz/image/upload/v1789048142/frenchworker_hu2pdi.png" alt="Sparks flying as a worker welds metal, representing Talent Passport manufacturing roles" loading="lazy" /></div>
              ...
              <h3>Skilled Employment</h3>
              <p style={{fontSize: '15.5px', color: 'rgba(255,255,255,.7)'}}>A fast-growing route into France's manufacturing, hospitality, caregiving and food-service sectors. Most fields ask for the TCF or DELF A2, plus a sector-specific skills test.</p>
              <ul>
                <li>French coaching timed to your target sector's test date</li>
            ...
            <div className="path-card">
              <div className="path-photo-wrap"><img className="path-photo" src="https://res.cloudinary.com/dzv9zcrlz/image/upload/v1789116129/france_whitecollar_rdmafq.png" alt="Engineers collaborating in a French technology workplace" loading="lazy" /></div>
              <div className="path-icon">
              ...
              <p style={{fontSize: '15.5px', color: 'rgba(255,255,255,.7)'}}>France's most common work visa for IT, engineering, marketing and international-business roles. As of April 2026, DELF B2 is now required by many employers for this category.</p>
              <ul>
                <li>Business French coaching aimed at B2 and DFP</li>
                <li>Résumé and interview preparation for French employers</li>
            ...
            <div className="path-card">
              <div className="path-photo-wrap"><img className="path-photo" src="https://res.cloudinary.com/dzv9zcrlz/image/upload/v1789116570/studyinfrance_ubvhjm.png" alt="Students walking through a French university campus" loading="lazy" /></div>
              <div className="path-icon">
              ...
              <h3>Study in France</h3>
              <p style={{fontSize: '15.5px', color: 'rgba(255,255,255,.7)'}}>From short-term language schools to full degree programmes. French universities increasingly test the Campus France alongside DELF for international admissions.</p>
              <ul>
                <li>DELF and Campus France-focused exam coaching</li>
      ...
      <section id="faq">
        <div className="wrap">
          <div className="sec-head reveal">
            <h2>Common questions about our French language course.</h2>
          </div>
          <div className="faq-list">
            <details className="faq-item">
              <summary>Do I need any prior knowledge of French to join?</summary>
              <p className="faq-a">No, most of our learners start at A1 with zero French. Classes are structured so complete beginners build confidence, and the French alphabet, from the very first session.</p>
            </details>
            <details className="faq-item">
              <summary>Is French pronunciation hard to get right?</summary>
              <p className="faq-a">French has some tricky sounds, silent letters and liaisons, but you'll get comfortable with the core pronunciation rules within the first few weeks. From there, vocabulary and grammar build up gradually and cumulatively from A1 through C1, so it never feels like a wall.</p>
            </details>
            <details className="faq-item">
            ...
            <details className="faq-item">
              <summary>Can I also prepare for DELF, TCF or DFP?</summary>
              <p className="faq-a">Yes, once you're at the right level, we offer focused certificate preparation for DELF, TCF, TEF, DALF and DFP alongside the regular course, for anyone who needs a specific score for a visa, job or university application.</p>
            </details>
            <details className="faq-item">
            ...
            <details className="faq-item"><summary>Can I start French from zero?</summary><p className="faq-a">Yes. The pathway begins at A1, and the programme is structured for learners with no prior French knowledge as well as learners joining at a higher level.</p></details>
            <details className="faq-item"><summary>How do I know which level I should join?</summary><p className="faq-a">A counsellor can help identify the appropriate starting point based on your previous French study, current ability and objective.</p></details>
            <details className="faq-item"><summary>Do you offer a free demo class?</summary><p className="faq-a">You can request a French demo or speak with a counsellor to understand the teaching format, level and current batch options.</p></details>
            <details className="faq-item"><summary>Can you help me choose a French exam?</summary><p className="faq-a">Yes. Counselling can help you identify whether DELF, TCF, DFP, Campus France or another assessment is relevant to your intended study or career pathway.</p></details>
            <details className="faq-item"><summary>Do you provide France career or visa guidance?</summary><p className="faq-a">France pathway support is available for the routes described on this page, including Talent Passport, Engineer/Specialist and Study in France pathways. Exact eligibility and immigration decisions depend on the applicable requirements.</p></details>
      ...
      {/* STUDENT ACTIVITIES / FRANCE CONNECTIONS */}
      <section className="activities" id="activities">
        <div className="tricolor-grid-dark" aria-hidden="true"></div>
        <div className="wrap">
          <div className="sec-head reveal">
            <div className="activity-eyebrow" style={{fontFamily: "'Roboto', sans-serif", fontSize: '11px', letterSpacing: '.14em', color: 'var(--gold-soft)', textTransform: 'uppercase', marginBottom: '10px'}}>Activities &amp; France Experience</div>
            <h2>Learn. Experience. Connect.</h2>
            <p>Learning French at Langma goes beyond classroom lessons. Build confidence through interactive activities, cultural experiences and real-life French practice.</p>
          </div>
      ...
      {/* FINAL CTA */}
      <section className="final-cta" id="start"><div className="tricolor-grid-dark" aria-hidden="true"></div><div className="wrap"><span className="kicker">Your French journey starts here</span><h2>Don't just learn French. Learn where it can take you.</h2><p>Get the current course fee, batch timings, learning mode and recommended level, without committing to a course first.</p><div className="final-cta-actions"><a className="btn btn-wa" href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20the%20French%20course%20fee%20and%20batch%20details." target="_blank" rel="noopener">Get Course Details on WhatsApp →</a><a className="btn btn-ghost" href="tel:+919810117094">Talk to a French Counsellor</a></div><div className="microcopy">No price displayed here. Ask us for the current course options, fees &amp; batch availability.</div></div></section>
          ...
          <div className="contact-grid">
            ...
            <div className="enroll-card">
              ...
              <form id="enrollForm" onSubmit={handleSubmit}>
                ...
                <input type="hidden" name="message" value="French Language Course enquiry" />
                <input type="hidden" name="type" value={FRENCH_FORM_CONFIG.type} />
                <input type="hidden" name="service" value={FRENCH_FORM_CONFIG.service} />
              ...
              <div className="form-alt">
                ...
                <a href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20to%20enroll%20for%20the%20French%20course." target="_blank" rel="noopener" style={{fontSize: '14.5px', fontWeight: '600', color: 'var(--gold-soft)', textDecoration: 'underline'}}>Message us on WhatsApp →</a>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* FRENCH FOOTER */}
        ...
        <div className="wrap">
          <div className="footer-grid">
            ...
            <br /><div className="fr-motto">Learn French. Understand France. Build Your Future.</div><p>Learn French with structured A1–C2 learning, practical communication, cultural activities and France-focused counselling.</p></div>
            <div className="footer-col"><h4>Learn</h4><a href="#course-details">French Course</a><a href="#skills">What You Learn</a><a href="#methodology">Teaching Method</a><a href="#batches">Batches</a></div>
            <div className="footer-col"><h4>France</h4><a href="#activities">Student Activities</a><a href="#pathways">France Pathways</a><a href="#careers">Career Opportunities</a><a href="#exam-guide">DELF / TCF / DFP</a></div>
          ...
          <div className="footer-bottom"><span>© {new Date().getFullYear()} Langma International Pvt. Ltd.</span><span>FRENCH LANGUAGE · {locationProfile.city.toUpperCase()}</span></div>
        </div>
      </footer>
      {/* FLOATING CALL */}
      ...
      {showRequestPopup && (
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999}} onClick={closeRequestPopup}>
          <div style={{background: '#fbf8f1', borderRadius: '16px', padding: '40px 32px', maxWidth: '420px', width: '90%', boxShadow: '0 28px 60px rgba(0,0,0,0.2)', animation: 'slideUp 0.3s ease'}} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{fontSize: '14px', color: '#514840', marginBottom: '24px'}}>We'll match you with the perfect French learning path</p>
      ...
      {/* FLOATING WHATSAPP */}
      <a href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20to%20know%20more%20about%20the%20French%20course." target="_blank" rel="noopener" className="wa-float" id="waFloat" aria-label="Chat on WhatsApp">
      ...
      <div className="mobile-cta-bar" aria-label="Quick contact buttons">
        ...
        <div className="mobile-cta-bar-actions">
          <a className="mobile-cta-bar-wa" href="https://wa.me/919810117094?text=Hi%20Langma%2C%20I%27d%20like%20to%20know%20more%20about%20the%20French%20course." target="_blank" rel="noopener" aria-label="Chat with Langma on WhatsApp">
